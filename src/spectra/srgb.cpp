...
struct SRGBSpectrum final : public Spectrum {
    ...
    [[nodiscard]] float4 encode_srgb_unbounded(float3 rgb) const noexcept override { return make_float4(rgb, 1.f); }
...
struct SRGBSpectrumInstance final : public Spectrum::Instance {
    ...
    [[nodiscard]] Float4 backward_decode_unbounded(
        const SampledWavelengths &swl, Expr<float4> rgb,
        const SampledSpectrum &dSpec) const noexcept override {
        return make_float4(dSpec[0u], dSpec[1u], dSpec[2u], 0.f);
    }
    ...
    [[nodiscard]] Float3 backward_encode_srgb_unbounded(Expr<float4> dEnc) const noexcept override {
        return dEnc.xyz();
    }
};
