...
float MitchellNetravaliFilter::_weight(float offset) const noexcept {
    auto x = std::min(std::abs(2.0f * offset), 2.0f);
    auto xx = x * x;
    return (1.0f / 6.0f) *
           (x > 1.0f ?
            ((-_b - 6.0f * _c) * xx + (6.0f * _b + 30.0f * _c) * x + (-12.0f * _b - 48.0f * _c)) * x + (8.0f * _b + 24.0f * _c) :
            ((12.0f - 9.0f * _b - 6.0f * _c) * xx + (-18.0f + 12.0f * _b + 6.0f * _c) * x) * x + (6.0f - 2.0f * _b));
}
