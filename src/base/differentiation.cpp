...
Differentiation::Differentiation(Pipeline &pipeline) noexcept
    ...
    Kernel1D apply_grad_const = [](BufferUInt gradients, BufferFloat4 params, BufferFloat2 ranges, Float alpha) noexcept {
        ...
        auto next = fma(-alpha, clamp(grad, -1e2f, 1e2f), old);
        params.write(thread, clamp(next, range.x, range.y));
    };
    Kernel2D apply_grad_tex = [](BufferUInt gradients, UInt offset, ImageFloat image, UInt channels, Float alpha, Float2 range) noexcept {
        ...
        image.write(coord, clamp(fma(-alpha, grad, old), range.x, range.y));
    };
...
Differentiation::TexturedParameter Differentiation::parameter(const Image<float> &image, TextureSampler s, float2 range) noexcept {
    ...
    TexturedParameter param{image, s, grad_offset, range};
...
void Differentiation::clear_gradients(CommandBuffer &command_buffer) noexcept {
    ...
    if (auto n = _gradient_buffer_size) {
        command_buffer << _clear_grad(*_grad_buffer).dispatch(n);
    }
}
...
void Differentiation::apply_gradients(CommandBuffer &command_buffer, float alpha) noexcept {
    ...
    if (auto n = _constant_params.size()) {
        ...
        command_buffer << _const_param_buffer.subview(0u, n).copy_to(params_before.data())
                       ...
                       << _apply_grad_const(*_grad_buffer, _const_param_buffer, _const_param_range_buffer, alpha)
                              .dispatch(n)
    ...
    for (auto &&p : _textured_params) {
        ...
        command_buffer << _apply_grad_tex(*_grad_buffer, offset, image, channels, alpha, p.range())
                              .dispatch(image.size());
    }
}
...
void Differentiation::accumulate(const Differentiation::TexturedParameter &param, Expr<float2> p, Expr<float4> grad) const noexcept {
    ...
    auto pixel_count = param.image().size().x * param.image().size().y;
    write_grad(map_uv(p), static_cast<float>(pixel_count) * grad);
}
