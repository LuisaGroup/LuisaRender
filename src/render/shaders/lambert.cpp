...
public:
    ...
    [[nodiscard]] static Scattering evaluate(Expr<float2> uv [[maybe_unused]], Expr<float3> n, Expr<float3> wo, Expr<float3> wi, Expr<float2> u2, Expr<Data> data, uint comp) {
        ...
        if (comp & EVAL_BSDF) {
            ...
            Var pdf = select(is_refl && valid, abs(cos_i) * inv_pi, 0.0f);
        ...
        if (comp & EVAL_BSDF_SAMPLING) {
            ...
            Var sampled_wi = sign(cos_i) * normalize(transform_to_world(onb, local_wi));
...
public:
    ...
    [[nodiscard]] static Emission emission(Expr<float2> uv [[maybe_unused]], Expr<float3> n, Expr<float3> w, Expr<Data> data) {
