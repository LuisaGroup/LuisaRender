...
public:
    enum Component {
        ...
        COMPONENT_PI = 1u << 1u,
        COMPONENT_HIT_TO_RAY_ORIGIN = 1u << 2u,
        COMPONENT_NG = 1u << 3u,
        COMPONENT_NS = 1u << 4u,
        COMPONENT_UV = 1u << 5u,
        COMPONENT_MATERIAL = 1u << 6u,
        COMPONENT_PDF = 1u << 7u,
...
private:
    ...
    BufferView<float3> _hit_to_ray_origin;
...
public:
    void create(Device *device, size_t size, uint32_t components = COMPONENT_ALL) noexcept {
        ...
        _hit_to_ray_origin = has_hit_to_ray_origin() ? device->allocate_buffer<float3>(size) : BufferView<float3>{};
    ...
    [[nodiscard]] bool has_hit_to_ray_origin() const noexcept { return _components & COMPONENT_HIT_TO_RAY_ORIGIN; }
    ...
    [[nodiscard]] auto &hit_to_ray_origin() noexcept { return _hit_to_ray_origin; }
    ...
    [[nodiscard]] const auto &hit_to_ray_origin() const noexcept { return _hit_to_ray_origin; }
