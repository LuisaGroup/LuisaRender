...
struct LambertMaterial : public Material {
    ...
    LambertMaterial(Device *device, const ParameterSet &params) : Material{device, params} {
        ...
        if (any(albedo != 0.0f)) { _lobes.emplace_back(shading::create_lambert_reflection(albedo, double_sided), 1.0f); }
        if (any(emission != 0.0f)) { _lobes.emplace_back(shading::create_lambert_emission(emission, double_sided), 1.0f); }
    }
};
