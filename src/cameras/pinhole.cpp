...
private:
    ...
    [[nodiscard]] Expr<RaySample> _generate_rays(Var<float4x4> camera_to_world, Var<float2> u [[maybe_unused]], Var<float2> pixel) override {
        ...
        Var<RaySample> sample;
        sample.ray.origin_x = o_world.x;
        sample.ray.origin_y = o_world.y;
        sample.ray.origin_z = o_world.z;
        sample.ray.min_distance = 1e-3f;
        sample.ray.direction_x = d.x;
        sample.ray.direction_y = d.y;
        sample.ray.direction_z = d.z;
        sample.ray.max_distance = 1e3f;
        sample.throughput = make_float3(1.0f);
        return sample;
    }
