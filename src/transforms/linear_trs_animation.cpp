...
float4x4 LinearTRSAnimation::matrix(float time) const noexcept {

    LUISA_EXCEPTION_IF(time < _key_frames.front().time_point || time > _key_frames.back().time_point, "Time point not in range: ", time);

    if (time == _key_frames.front().time_point) { return _key_frames.front().transform->matrix(time); }
    if (time == _key_frames.back().time_point) { return _key_frames.back().transform->matrix(time); }
...
LUISA_EXPORT_PLUGIN_CREATOR(luisa::render::transform::LinearTRSAnimation)
