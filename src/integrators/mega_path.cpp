    ...
    [[nodiscard]] Float3 Li(const Camera::Instance *camera, Expr<uint> frame_index,
        ...
        $for(depth, node<MegakernelPathTracing>()->max_depth()) {
            ...
            if (use_ser) {
                ...
                // 3. has surface
                // 4. surface tag
                // 5. whether this path is going to terminate
                //    this condition cannot be (fully) known before evaluating bsdf,
                //    due to russian roulette, so we just guess by applying some threshold
                ...
                $else {
                    Shape::Handle shape = pipeline().geometry()->instance(closest_hit.inst);
