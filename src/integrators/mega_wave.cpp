...
class MegaWavefrontPathTracing final : public Integrator {
...
public:
    MegaWavefrontPathTracing(Scene *scene, const SceneNodeDesc *desc) noexcept
...
class MegaWavefrontPathTracingInstance final : public Integrator::Instance {
...
public:
    explicit MegaWavefrontPathTracingInstance(const MegaWavefrontPathTracing *node, Pipeline &pipeline) noexcept
        : Integrator::Instance{pipeline, node}, _pipeline{pipeline} {}
    void render(Stream &stream) noexcept override {
        auto pt = static_cast<const MegaWavefrontPathTracing *>(node());
...
unique_ptr<Integrator::Instance> MegaWavefrontPathTracing::build(Pipeline &pipeline, CommandBuffer &) const noexcept {
    return luisa::make_unique<MegaWavefrontPathTracingInstance>(this, pipeline);
}

void MegaWavefrontPathTracingInstance::_render_one_camera(
    ...
    Kernel1D render_kernel = [&](UInt frame_offset, UInt max_path_count, BufferUInt counter,
                                 Float4x4 camera_to_world, Float3x3 camera_to_world_normal,
                                 Float3x3 env_to_world, Float time, Float shutter_weight) noexcept {
        set_block_size(64u, 1u, 1u);

        $loop {

            auto path_id = counter.atomic(0u).fetch_add(1u);
            $if(path_id >= max_path_count) { $break; };

            auto sample_count = max_path_count / pixel_count;
            auto position_id = path_id / sample_count;
            auto sample_id = path_id % sample_count + frame_offset;
            auto pixel_id = make_uint2(
                position_id % resolution.x,
                position_id / resolution.x);
            sampler->start(pixel_id, sample_id);
    ...
    auto sample_offset = 0u;
    auto worker_count = 65536u;
    auto count_buffer = pipeline.device().create_buffer<uint>(1u);
    for (auto s : shutter_samples) {
        ...
        auto zero = 0u;
        static constexpr auto samples_per_dispatch = 4u;
        for (auto i = 0u; i < s.spp; i += samples_per_dispatch) {
            auto sample_count = std::min(s.spp, i + samples_per_dispatch) - i;
            auto path_count = pixel_count * sample_count;
            command_buffer << count_buffer.copy_from(&zero)
                           << render(sample_offset, path_count, count_buffer,
                                     camera_to_world, camera_to_world_normal,
...
LUISA_RENDER_MAKE_SCENE_NODE_PLUGIN(luisa::render::MegaWavefrontPathTracing)
