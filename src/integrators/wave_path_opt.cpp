//
// Created by Mike Smith on 2022/8/23.
//
...
public:
    LightSelectionSOA(Device &device, size_t size) noexcept {
        _tag = device.create_buffer<uint>(size);
        _prob = device.create_buffer<float>(size);
        _u = device.create_buffer<float2>(size);
    }

    [[nodiscard]] auto read_tag(Expr<uint> index) const noexcept { return _tag.read(index); }
    void write_tag(Expr<uint> index, Expr<uint> tag) noexcept { _tag.write(index, tag); }
    [[nodiscard]] auto read_prob(Expr<uint> index) const noexcept { return _prob.read(index); }
    void write_prob(Expr<uint> index, Expr<float> prob) noexcept { _prob.write(index, prob); }
    [[nodiscard]] auto read_u(Expr<uint> index) const noexcept { return _u.read(index); }
    void write_u(Expr<uint> index, Expr<float2> u) noexcept { _u.write(index, u); }
};
...
void WavefrontPathTracingInstance::_render_one_camera(
    ...
    LightSelectionSOA light_selections{device, state_count};
    ...
    LUISA_INFO("Compiling light selection kernel.");
    auto select_light_shader = device.compile_async<1>([&](BufferUInt path_indices, BufferRay rays, BufferHit hits,
                                                           BufferUInt queue, BufferUInt queue_size, Float time) {
        auto queue_id = dispatch_x();
        $if(queue_id < queue_size.read(0u)) {
            auto ray_id = queue.read(queue_id);
            auto ray = rays.read(ray_id);
            ...
            LightSampler::Selection sel = light_sampler()->select(*it, u_light_selection, swl, time);
            light_selections.write_tag(queue_id, sel.tag);
            light_selections.write_prob(queue_id, sel.prob);
            light_selections.write_u(queue_id, u_light_surface);
        };
    });
    ...
    auto sample_light_shader = device.compile_async<1>([&](BufferUInt path_indices, BufferRay rays, BufferHit hits,
        ...
        $if(queue_id < queue_size.read(0u)) {
            ...
            auto path_id = path_indices.read(ray_id);
            auto swl = path_states.read_swl(path_id);
            auto u_light_surface = light_selections.read_u(queue_id);
            auto tag = light_selections.read_tag(queue_id);
            auto prob = light_selections.read_prob(queue_id);
            LightSampler::Selection sel{tag, prob};
            auto light_sample = light_sampler()->sample_selection(*it, sel, u_light_surface, swl, time);
            // trace shadow ray
    ...
    for (auto s : shutter_samples) {
        ...
        for (auto i = 0u; i < s.spp; i += spp_per_launch) {
            ...
            for (auto depth = 0u; depth < node<WavefrontPathTracing>()->max_depth(); depth++) {
                ...
                command_buffer << select_light_shader.get()(path_indices, rays, hits, surface_indices, surface_count, time)
                                      .dispatch(launch_state_count)
                               << sample_light_shader.get()(path_indices, rays, hits, surface_indices, surface_count, time)
                                      .dispatch(launch_state_count)
