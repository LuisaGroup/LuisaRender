...
constexpr auto texture_jit_header =
    ...
    "        auto pixel = surf2Dread<::float>(t.texture, coord.x * 4u, coord.y);\n"
    "        return make_float4(pixel, 0.0f, 0.0f, 0.0f);\n"
    "    } else if constexpr (is_same_v<T, float2>) {\n"
    "        auto pixel = surf2Dread<::float2>(t.texture, coord.x * 8u, coord.y);\n"
    "        return make_float4(pixel.x, pixel.y, 0.0f, 0.0f);\n"
    "    } else if constexpr (is_same_v<T, float4>) {\n"
    "        auto pixel = surf2Dread<::float4>(t.texture, coord.x * 16u, coord.y);\n"
    "        return make_float4(pixel.x, pixel.y, pixel.z, pixel.w);\n"
    "    } else if constexpr (is_same_v<T, uint8_t>) {\n"
    "        auto pixel = surf2Dread<uint8_t>(t.texture, coord.x, coord.y);\n"
    "        return make_float4(pixel / 255.0f, 0.0f, 0.0f, 0.0f);\n"
    "    } else if constexpr (is_same_v<T, uchar2>) {\n"
    "        auto pixel = surf2Dread<::uchar2>(t.texture, coord.x * 2u, coord.y);\n"
    "        return make_float4(pixel.x / 255.0f, pixel.y / 255.0f, 0.0f, 0.0f);\n"
    "    } else if constexpr (is_same_v<T, uchar4>) {\n"
    "        auto pixel = surf2Dread<::uchar4>(t.texture, coord.x * 4u, coord.y);\n"
    "        return make_float4(pixel.x / 255.0f, pixel.y / 255.0f, pixel.z / 255.0f, pixel.w / 255.0f);\n"
    "    }\n"
    ...
    "        surf2Dwrite<::float2>(p, t.texture, coord.x * 8u, coord.y);\n"
    "    } else if constexpr (is_same_v<T, float4>) {\n"
    "        auto p = ::float4{pixel.x, pixel.y, pixel.z, pixel.w};\n"
    "        surf2Dwrite<::float4>(p, t.texture, coord.x * 16u, coord.y);\n"
    "    } else if constexpr (is_same_v<T, uint8_t>) {\n"
    "        auto p = static_cast<uint8_t>(clamp(pixel.x * 255.0f, 0.0f, 255.0f));\n"
    "        surf2Dwrite<uint8_t>(p, t.texture, coord.x, coord.y);\n"
    "    } else if constexpr (is_same_v<T, uchar2>) {\n"
    "        auto p = ::uchar2{\n"
    "            static_cast<uint8_t>(clamp(pixel.x * 255.0f, 0.0f, 255.0f)),\n"
    "            static_cast<uint8_t>(clamp(pixel.y * 255.0f, 0.0f, 255.0f))};\n"
    "        surf2Dwrite<::uchar2>(p, t.texture, coord.x * 2u, coord.y);\n"
    "    } else if constexpr (is_same_v<T, uchar4>) {\n"
    "        auto p = ::uchar4{\n"
    "            static_cast<uint8_t>(clamp(pixel.x * 255.0f, 0.0f, 255.0f)),\n"
    "            static_cast<uint8_t>(clamp(pixel.y * 255.0f, 0.0f, 255.0f)),\n"
    "            static_cast<uint8_t>(clamp(pixel.z * 255.0f, 0.0f, 255.0f)),\n"
    "            static_cast<uint8_t>(clamp(pixel.w * 255.0f, 0.0f, 255.0f))};\n"
    "        surf2Dwrite<::uchar4>(p, t.texture, coord.x * 4u, coord.y);\n"
    "    }\n"
    "}\n"
    "\n"
    "}\n";
