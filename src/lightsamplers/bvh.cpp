...
[[nodiscard]] luisa::vector<QBVHNode> encode_quantized_bvh(BVHNode *root, float3 world_min, float3 world_max) noexcept {
    ...
    while (!stack.empty()) {
        ...
        if (!node->is_leaf()) {
            ...
            stack.push({node->left_child(), current_index});
        }
    }
...
LUISA_STRUCT(luisa::render::QBVHNode, quantized_p_min, quantized_p_max, power, parent, right_child, first_primitive_offset, primitive_count, lut_entry){};
...
private:
    ...
    Buffer<float3> _world_bounds_buffer;
    Buffer<BVHPrimitive> _bvh_primitive_buffer;
    Buffer<QBVHNode> _quantized_bvh_node_buffer;
...
public:
    BVHLightSamplerInstance(const BVHLightSampler *sampler, Pipeline &pipeline, CommandBuffer &command_buffer)
        ...
        if (!pipeline.lights().empty()) {
            ...
            _world_bounds_buffer = pipeline.device().create_buffer<float3>(2u);
            _bvh_primitive_buffer = pipeline.device().create_buffer<BVHPrimitive>(n);
            _quantized_bvh_node_buffer = pipeline.device().create_buffer<QBVHNode>(2u * n - 1u);
            _update_primitive_data = luisa::make_shared<Shader1D<float>>(pipeline.device().compile<1>([&](Float time) noexcept {
                set_block_size(256u);
                auto n = static_cast<uint>(pipeline.geometry()->light_instances().size());
                auto tag = dispatch_id().x;
                $while(tag < n) {
                    ...
                    $for(primitive_id, 0u, light_inst.triangle_count()) {
                    ...
                    primitive.x_min = p_min.x;
                    primitive.y_min = p_min.y;
                    primitive.z_min = p_min.z;
                    primitive.x_max = p_max.x;
                    primitive.y_max = p_max.y;
                    primitive.z_max = p_max.z;
                    primitive.power = power;
                    primitive.tag = tag;
                    _bvh_primitive_buffer->write(tag, primitive);
                    tag += dispatch_size().x;
                };
            }));
        }
    ...
    void update(CommandBuffer &command_buffer, float time) noexcept override {
        if (!pipeline().lights().empty()) {
            ...
            command_buffer << (*_update_primitive_data)(time).dispatch(1024u)
                           << _bvh_primitive_buffer.copy_to(primitive_buffer.data())
                           << commit()
                           << synchronize();
            luisa::list<BVHPrimitive> primitive_list;
            for (auto const &primitive : primitive_buffer) {
            ...
            command_buffer << _world_bounds_buffer.copy_from(world_bounds.data())
            ...
            if (quantized_nodes.size() < 2u * n - 1u) {
                quantized_nodes.resize(2u * n - 1u);
            }
            for (auto i = 0u; i < quantized_nodes.size(); i++) {
                if (quantized_nodes[i].right_child == 0xffffffffu) {
                    ...
                    for (auto j = first_primitive_offset; j < last_primitive_offset; j++) {
                        ...
                        quantized_nodes[tag].lut_entry = i;
                    }
                }
            }
