...
LUISA_CONSTANT_SPACE constexpr auto SPECULAR = static_cast<uint8_t>(0x10u);
LUISA_CONSTANT_SPACE constexpr auto BACK_FACE = static_cast<uint8_t>(0x20u);
...
private:
    ...
    std::unique_ptr<Buffer<float4>> _emission_buffer;
...
public:
    ...
    InteractionBufferSet(Device *device, size_t capacity, uint flags = interaction::attribute::ALL)
        : _size{capacity},
          ...
          _wo_and_pdf_buffer{(flags & interaction::attribute::WO_AND_PDF) ?
          ...
          _emission_buffer{(flags & interaction::attribute::EMISSION) ? device->create_buffer<float4>(capacity, BufferStorage::DEVICE_PRIVATE) : nullptr} {}
